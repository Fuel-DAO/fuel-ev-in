...
pub fn HomeBanner(is_root: bool) -> impl IntoView {
    ...
    let root_class = if is_root {
        "h-screen bg-cover bg-center"
    } else {
        " h-1/2 bg-cover bg-center"
    };
    view! {
        <div class=move || root_class style=format!("background-image: url('{}');", image_url)>
            <div class="absolute inset-0 bg-black opacity-50"></div>

            <div class="absolute flex flex-col w-full">

                <div class="flex h-16 items-center" style="margin-left:10px; margin-right:10px">
...
pub fn SearchBar(is_root: bool) -> impl IntoView {
    ...
    view! {
        <div class=class>
            ...
            <a
                href="/search"
                class="bg-green-500 text-center text-white rounded-lg px-8 py-2 w-full md:w-auto hover:bg-green-600"
            >
                Search
            </a>
        </div>
